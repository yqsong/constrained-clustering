...
public class SparseGraphWeightedMST extends SparseGraph{
	...
	public SparseGraphWeightedMST(int vertexNum) {
		super(vertexNum);
		...
		{                      // put currentVert in tree
			...
			for(int j=0; j<nVerts; j++)   // for each vertex,
			{
				...
				if(this.vertexList[j].isInTree) // skip if in the tree
					continue;
				double distance = adjMat.get(currentVert).get(j);
	...
	{
		SparseGraphWeightedMST theGraph = new SparseGraphWeightedMST(6);
